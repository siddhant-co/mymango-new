        ...
        <div key={index} className="relative w-full">
          {banner.image && process.env.NEXT_PUBLIC_API_BASE_URL && (
            <Image
              ...
              width={1920}
              height={1080}
              className="rounded-none object-cover w-full h-[50vh] sm:h-[70vh] md:h-[52vh] lg:h-auto"
            />
          )}

          {/* Heading */}
          {banner.heading && (
            <p className="absolute top-32 sm:top-[80px] md:top-[180px] left-6 sm:left-16 text-left text-xl sm:text-3xl md:text-4xl font-semibold text-white">
              {banner.heading}
            </p>
          )}
          ...
          {/* Description */}
          {banner.description && (
            <p className="absolute top-[11rem] sm:top-[10rem] md:top-[250px] left-6 sm:left-16 text-left text-xs sm:text-sm md:text-lg text-white font-thin pr-6 sm:pr-[30px] max-w-[1000px]">
              <span className="text-sm sm:text-lg md:text-[30px] text-white font-medium">
                {banner.description}
              </span>
            </p>
          )}
